...
import { CustomStep, OptionsStep, TextStep, SelectionsStep } from './steps';
...
class ChatBot extends Component {
  ...
  triggerNextStep(data) {
    ...
    if (isEnd) {
    ...
    } else if (currentStep.selections && data) {
      console.log("current step is selection");
      const selection = currentStep.selections.filter(o => o.value === data.value)[0];
      const trigger = this.getTriggeredStep(selection.trigger, currentStep.value);
      delete currentStep.selections;
      ...
      this.setState({ renderedSteps, currentStep, previousStep }, () => {
        ...
        else if (nextStep.selections && nextStep.selections instanceof Function) {
          nextStep.selections = nextStep.selections();
  ...
  renderStep(step, index) {
    ...
    const { selections, options, component, asMessage } = step;
    const steps = this.generateRenderedStepsById();
